...
def synthetic_bundle(*, runs: int = 12, seed: int = 0) -> bytes:
    ...
    for run_index in range(runs):
        ...
        for step, surface in enumerate(SURFACE_EXAMPLES):
            ...
            actions = tuple(
                SemanticAction(
                    key=f"opaque-action-{run_index}-{step}-{index}",
                    kind="choose_option",
                    subject=FrozenObject.of({"definition": f"fixture-option-{index}"}),
                    arguments=FrozenObject.of(
                        {"visible_cost": index, "visible_effect": index + step % 2}
                    ),
                )
            ...
            read = AdmittedRead(
                "run_deck",
                FrozenObject.of({"cards": ["fixture-card"]}),
                semantic_hash([run_id, step, "read"]),
            )
            state = SemanticState(
                run,
                FrozenObject.of({"surface": surface, "domain": "gameplay"}),
                tuple(action.subject for action in actions),
                (read,),
            )
            successor = SemanticState(
                run, FrozenObject.of({"resolved": True, "floor": step}), (), ()
            )
            records.append(
                {
                    "record_id": f"record-{run_index}-{step}",
                    "run_id": run_id,
                    "episode_id": run_id,
                    "step_index": step,
                    "domain": "gameplay",
                    "family": "synthetic_choice",
                    "surface": surface,
                    "state": state.to_dict(),
                    "actions": [action.to_dict() for action in actions],
                    "catalog_complete": True,
                    "catalog_count": len(actions),
                    "chosen_key": actions[-1].key,
                    "successor": successor.to_dict(),
                    "terminal": step == len(SURFACE_EXAMPLES) - 1,
                    "disposition": "committed",
                }
            )
        proofs[run_id] = {
            "started_ref": semantic_hash([run_id, "start"]),
    ...
    return json_bytes(
        {
            ...
            "environment_identity": semantic_hash(
...
class SyntheticSourceAdapter:
    ...
    def project(self, raw: bytes) -> SourceProjection:
        obj = object_fields(
            decode_json(raw),
            {"schema", "environment_identity", "run_proofs", "records"},
            "synthetic_source",
        )
        ...
        for raw_record in array(obj["records"], "source.records"):
            ...
            provenance = EvidenceLink(
                ...
                environment,
                "engineering",
                semantic_hash([self.adapter_id, bundle_id]),
                f"synthetic-root-{record_id}",
        ...
        for run_id in run_ids:
            proof = object_fields(
                obj["run_proofs"][run_id],
                {"started_ref", "terminal_ref", "record_count"},
                "run_proof",
            )
            digest(proof["started_ref"], "run.start")
            digest(proof["terminal_ref"], "run.terminal")
            run_records = sorted(
                (r for r in records if r.run_id == run_id), key=lambda r: r.step_index
            )
            count = unsigned(proof["record_count"], "run.record_count")
            if count != len(run_records) or [r.step_index for r in run_records] != list(
                range(count)
            ):
                raise BoundaryError("source", "missing_or_duplicate_disposition")
            if (
                not run_records
                or not run_records[-1].terminal
                or any(r.terminal for r in run_records[:-1])
            ):
        ...
        return SourceProjection(
            ...
            bundle_id,
            "engineering",
            FrozenObject.of(obj["run_proofs"]),
            tuple(records),
        )
