...
def _publish_temporary(temporary: Path, destination: Path) -> None:
    """Publish verified bytes once; never replace an existing immutable object."""
    try:
        os.link(temporary, destination)
    except FileExistsError:
        if destination.is_symlink() or not destination.is_file():
            raise TrainingHandoffError("immutable destination is not a regular file") from None
        if temporary.stat().st_size != destination.stat().st_size or (
            _sha256(temporary) != _sha256(destination)
        ):
            raise TrainingHandoffError(
                "immutable publication collision; refusing overwrite"
            ) from None


def _atomic_copy(source: Path, destination: Path, expected_digest: str | None = None) -> None:
    ...
    try:
        ...
        if expected_digest is not None and _sha256(temporary) != expected_digest:
            raise TrainingHandoffError("source changed during immutable publication")
...
def _logical_path(value: str) -> str:
    # Logical paths are POSIX protocol fields, never host filesystem spellings.
    if (
...
def _load_manifest(store: Path, training_input_id: str) -> dict[str, Any]:
    _object_path(store, training_input_id)  # validate before constructing a filesystem path
...
def verify_training_input(
    ...
    *,
    ...
    for value in objects:
        ...
        logical_path = _logical_path(str(item.get("logical_path", "")))
    ...
    *, source_store: str | Path, receiver_store: str | Path, training_input_id: str
    ...
    for value in _sequence(manifest.get("objects"), "training-input objects"):
        ...
        if destination.exists():
            ...
            reused += 1
            continue
        _atomic_copy(source_object, destination, digest)
        transferred += 1
    ...
    if receiver_manifest.exists():
        ...
        _atomic_copy(source_manifest, receiver_manifest, _sha256(source_manifest))
