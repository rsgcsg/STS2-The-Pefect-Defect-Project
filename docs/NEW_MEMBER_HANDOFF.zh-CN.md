...
> 本轮统一任务界面仍在集成与验证中，尚未部署、完成新的真人验收或正式发布。第 4–6 节的新界面流程适用于本轮候选；使用正式发布的成员继续按对应发布的安装说明操作，不因本指南更新重装。日常使用先看[统一任务流程](../python/docs/UNIFIED_TASK_FLOW.zh-CN.md)，本指南负责工程与运维交接。
...
本轮候选可以选择已有决策数据集，点 **合并选中数据集 · 先预览**，再生成新的固定版本。只合并父数据集已经选中的决策，按精确决策身份去重；相似局面不算重复，不会从原录制捞回父数据集排除的行。原数据集保留，新版本重新按整局分组划分；曾用于训练或评估的数据不会因此成为新的无偏测试集。
