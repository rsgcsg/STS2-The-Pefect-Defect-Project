...
组件源码用正常 merge commit，不能 squash/rebase 改写其 source provenance；纯文档按根规范可以 squash。正常修改先进入 develop，发布/hotfix 按治理进入 main，并按需要同步 develop。合并后验证实际 merge-head CI，才删除已合并主题分支；main/develop 是仅有的长期分支，并非禁止工作期间出现临时分支。
